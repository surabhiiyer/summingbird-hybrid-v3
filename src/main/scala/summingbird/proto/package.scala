...
package object proto {
  ...
  def dataFileForBatch(batch: BatchID) = {
    ...
    DataDir + "orders_0_" + DataFileDateFormat.format(batcher.earliestTimeOf(batch.next).toDate)
  }
  //consists of the product id, date and the json object that consists of details of the orders.
  ...
  def parseView(bytes: Array[Byte]): OrderViewed = {
    parseView(new String(bytes))
  }

  def parseView(s: String): OrderViewed = {
    val bits = s.split("\t")
    OrderViewed(bits(0).toLong, new Date(bits(1).toLong), bits(2))
  }

  def serializeView(orderView: OrderViewed): String = {
    "%s\t%s\t%s".format(orderView.productId, orderView.requestTime.getTime, orderView.orderjson)
  }
